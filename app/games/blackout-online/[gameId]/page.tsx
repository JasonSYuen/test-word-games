    ...
    <div className="relative min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100" onClick={handleClickOutside}>
      ...
      {!gameEnded && (
        <div className="md:hidden flex justify-around px-4 pb-2 relative" style={{ zIndex: 6 }}>
          ...
          <div className={`p-3 rounded-lg shadow flex-1 ml-2 ${myPlayerNumber === 2 ? 'bg-orange-100 dark:bg-orange-900' : 'bg-gray-100 dark:bg-gray-800'}`}>
      ...
      {!gameEnded && (
        <>
          ...
          <div className={`hidden md:block absolute top-20 right-4 p-4 rounded-lg shadow ${myPlayerNumber === 2 ? 'bg-orange-100 dark:bg-orange-900' : 'bg-gray-100 dark:bg-gray-800'}`} style={{ zIndex: 6 }}>
        ...
        <div className="relative w-full max-w-[500px]" onClick={(e) => e.stopPropagation()}>
          ...
          <svg className="absolute top-0 left-0 w-full h-full pointer-events-none" style={{ zIndex: 1 }}>
            {submittedWords.map((wordData, idx) => {
              const color = wordData.player === 1 ? '#3b82f6' : '#f97316';
              const lines = [];
