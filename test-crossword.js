// Test script to measure crossword generation performance
...
let POSITION_INDICES = null;

function getPositionIndices() {
  ...
  if (isRowStep && index < 5) {
    ...
    for (const word of shuffledCandidates) {
      ...
      if (attemptCount >= MAX_ATTEMPTS) {
        console.log(`Hit max attempts (${MAX_ATTEMPTS}), stopping this attempt...`);
        return false;
      }

      if (attemptCount % 10000 === 0) {
        console.log(`Attempt ${attemptCount}: Step ${step}, trying horizontal word "${word}" for row ${index}`);
      }
...
function generateCrossword() {
  ...
  while (retryCount < maxRetries) {
    ...
    if (retryCount > 0) {
      console.log(`\nRetry ${retryCount + 1}: Starting new generation attempt with different starting word...`);
    }
    ...
    if (fillGridAlternating(grid, 0, filledRows, filledCols, horizontalWords, verticalWords)) {
      ...
      console.log(`Success after ${attemptCount} attempts on retry ${retryCount + 1}!`);
      return { grid, horizontalWords, verticalWords, totalScore };
    }

    console.log(`Retry ${retryCount + 1} failed after ${attemptCount} attempts`);
    retryCount++;
  }
...
function printCrossword(solution) {
  console.log('\nGenerated Crossword:');
  console.log('═'.repeat(21));
  for (let row = 0; row < 5; row++) {
    console.log(solution.grid[row].join(' '));
  }
  console.log('═'.repeat(21));
  console.log('\nHorizontal Words:', solution.horizontalWords.join(', '));
  console.log('Vertical Words:', solution.verticalWords.join(', '));
  console.log('Total Score:', solution.totalScore);
}

// Run the test
console.log('Starting crossword generation test...\n');
...
for (let i = 0; i < attempts; i++) {
  ...
  if (result) {
    successCount++;
    console.log(`\nAttempt ${i + 1}: ✓ Success in ${time}ms`);
    console.log('─'.repeat(21));
    for (let row = 0; row < 5; row++) {
      console.log(result.grid[row].join(' '));
    }
    console.log('─'.repeat(21));
    console.log(`H: ${result.horizontalWords.join(', ')}`);
    console.log(`V: ${result.verticalWords.join(', ')}`);
    console.log(`Score: ${result.totalScore}`);
  } else {
    console.log(`Attempt ${i + 1}: ✗ Failed after ${time}ms`);
  }
}

console.log('\n' + '═'.repeat(50));
console.log(`Results: ${successCount}/${attempts} successful`);
console.log(`Average time: ${(totalTime / attempts).toFixed(2)}ms`);
